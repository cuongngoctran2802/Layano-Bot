...
module.exports = {
	...
	description: "Loại bỏ 1+ bài hát", //the command description for Slash Command Overview
	cooldown: 10,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		{
			"Integer": {
				name: "what_song",
				description: "Bạn muốn xóa Index bài hát nào?",
				required: true
			}
		}, //to use in the code: interacton.getInteger("ping_amount")
		{
			"Integer": {
				name: "how_many",
				description: "Bạn muốn xóa bao nhiêu bài hát từ đó? (Mặc định: 1)",
				required: false
			}
		}, //to use in the code: interacton.getInteger("ping_amount")
			...
			try {
				...
				if (songIndex > newQueue.songs.length - 1) return interaction.reply({
					embeds: [
						...
						.setDescription(`**Bài hát cuối cùng trong Hàng đợi có index: \`${newQueue.songs.length}\`**`)
					],
					ephemeral: true
				})
				if (songIndex <= 0) return interaction.reply({
					embeds: [
						...
						.setDescription(`**Sử dụng \`${client.settings.get(guild.id, "prefix")}skip\` để thay thế lệnh (Slash)!**`)
					],
					ephemeral: true
				})
				...
				interaction.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`🗑 **Đã loại bỏ ${amount} bài ${amount > 1 ?"s": ""} ra khỏi hàng đợi!**`)
