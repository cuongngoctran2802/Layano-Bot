...
module.exports = {
	...
	description: "Danh sách bài hát hiện tại", //the command description for Slash Command Overview
	cooldown: 10,
			...
			try {
				...
				for (let i = 0; i < theSongs.length; i += 10) {
					...
					const embed = new MessageEmbed()
						...
						.setDescription(`${info}`)
					if (i < 10) {
						embed.setTitle(`📑 **Top ${theSongs.length > 50 ? 50 : theSongs.length} | Hàng chờ của ${guild.name}**`)
						embed.setDescription(`**(0) Bài hát hiện tại:**\n> [\`${theSongs[0].name.replace(/\[/igu, "{").replace(/\]/igu, "}")}\`](${theSongs[0].url})\n\n${info}`)
					}
					embeds.push(embed);
					k += 10; //Raise k to 10
				}
				embeds[embeds.length - 1] = embeds[embeds.length - 1]
					.setFooter(ee.footertext + `\n${theSongs.length} Bài hát trong hàng đợi | Thời lượng: ${newQueue.formattedDuration}`, ee.footericon)
				let pages = []
				...
				const Menu = new MessageSelectMenu()
					...
					.addOptions([
						pages.map((page, index) => {
							let Obj = {};
							Obj.label = `Trang ${index}`
							Obj.value = `${index}`;
							Obj.description = `Hiện trang ${index}/${pages.length - 1}!`
							return Obj;
						})
					])
				...
				client.on('interactionCreate', (i) => {
					...
					if (i.customId === "trang" && i.applicationId == client.user.id) {
						i.reply({
