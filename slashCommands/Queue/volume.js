...
module.exports = {
	...
	description: "Điều chỉnh âm lượng của bài hát", //the command description for Slash Command Overview
	cooldown: 10,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		{
			"Integer": {
				name: "volume",
				description: "Âm lượng nên là gì? Nó phải nằm trong khoảng từ 0 đến 150",
				required: true
			}
		...
		try {
			...
			try {
				...
				if (volume > 150 || volume < 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Âm lượng phải ở giữa \`0\` và \`150\`!**`)
					],
					ephemeral: true
				})
				...
				interaction.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`🔊 **Đã thay đổi Âm lượng thành \`${volume}\`!**`)
