	...
	run: async (client, interaction) => {
		try {
			...
			await interaction.reply({
				content: `${client.allEmojis.loading} Đang tải **'${args[0] ? args[0] : "Default"}' nhạc Mix**`,
				ephemeral: true
			});
			try {
				...
				interaction.editReply({
					content: `${queue?.songs?.length > 0 ? "👍 Đã thêm" : "🎶 Đang phát"}: the **'${args[0] ? args[0] : "Default"}'**`,
					ephemeral: true
				});
			} catch (e) {
