...
module.exports = client => {
    //Start teh website
    console.log("Đang chạy cài đặt bảng điều khiển".brigthGreen)
    ...
    app.get(`/callback`, passport.authenticate(`discord`, { failureRedirect: "/" }), async (req, res) => {
        ...
        if(banned) {
                req.session.destroy(() => {
                res.json({ login: false, message: `Bạn đã bị chặn từ Bảng điều khiển.`, logout: true })
                req.logout();
            });
        } else {
    ...
    app.get("/dashboard", checkAuth, async (req,res) => {
      ...
      return res.redirect("/?error=" + encodeURIComponent("Bạn cần đăng nhập trước!"));
      if(!req.user.guilds)
      return res.redirect("/?error=" + encodeURIComponent("Không thể tìm thấy máy chủ của bạn!"));
    ...
    app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
      ...
      if (!guild) return res.redirect("/dashboard?error=" + encodeURIComponent("Không thể tìm thấy Thông tin dữ liệu máy chủ."));
      let member = guild.members.cache.get(req.user.id);
      if (!member) {
        try {
          member = await guild.members.fetch(req.user.id);
        } catch (err) {
          console.error(`Không tìm thấy ${req.user.id} ở ${guild.name}: ${err}`);
        }
      }
      if (!member) return res.redirect("/dashboard?error=" + encodeURIComponent("Không thể tìm thấy bạn, xin lỗi!"));
      if (!member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) {
        return res.redirect("/dashboard?error=" + encodeURIComponent("Bạn không được cho phép làm điều này!"));
      }
    ...
    app.post("/dashboard/:guildID", checkAuth, async (req, res) => {
      // We validate the request, check if guild exists, member is in guild and if member has minimum permissions, if not, we redirect it back.
      const guild = client.guilds.cache.get(req.params.guildID);
      if (!guild) return res.redirect("/dashboard?error=" + encodeURIComponent("Không thể lấy Thông tin dữ liệu Máy chủ!"));
      let member = guild.members.cache.get(req.user.id);
      if (!member) {
        try {
          member = await guild.members.fetch(req.user.id);
      ...
      if (!member) return res.redirect("/dashboard?error=" + encodeURIComponent("Không có thông tin dữ liệu về bạn!"));
    ...
    http.listen(settings.config.http.port, () => {
        console.log(`[${settings.website.domain}]: đang chạy web ở cổng ${settings.config.http.port}.`)
    });
}
